...
'''
리스트 : 1차원 데이터인 벡터나 서로 다른
        주고의 데이터를 그룹으로 묶은
        데이터 세트

데이터 프레임 : 이러한 리스트를 2차원으로
                확대한 것
'''
...
# seq()
var3 <- seq(1,5) # 1~5까지 연속값으로 var3 변수 생성
var3

var4 <- seq(1,1000, by=2) # 1~1000 까지 2간격 연속값으로 var4 생성
var4

var5 <- seq(0,100, by=2) # 1~100 까지 짝수만 생성
var5
...
함수 : 기능을 하는 수
-> 입력을 받아서 출력을 내는 수
-> 값을 넣으면 특정 기능을 수행해서 처음과 다른 값을 출력
'''
...
library(ggplot2) # 패키지 로드 명령어

### ggplot2 함수 사용 
x <- c('a','a','b','c')
qplot(x) # qplot() : 빈도 그래프 함수

'''
거의 모든 패키지에는 함수를 테스트할 수 있는 테스트용
데이터가 존재한다. 

ex) ggplot2 에는 mpg 데이터가 존재한다.
'''
